...
do
	...
	docker run -itd --net=hadoop \
                ...
                -p ${i}8088:8088 \
                -p ${i}9870:9870 \
                --name namenode$i --hostname namenode$i txhsl/hadoop-demo &> /dev/null

	echo "start zk$i container..."
	docker run -itd --net=hadoop \
                -p ${i}2181:2181 \
		--name zk$i --hostname zk$i txhsl/hadoop-demo &> /dev/null
	...
	docker run -itd --net=hadoop \
		--name jn$i --hostname jn$i txhsl/hadoop-demo &> /dev/null
...
do
	...
	docker run -itd --net=hadoop \
		--name worker$i --hostname worker$i txhsl/hadoop-demo &> /dev/null
	i=$(( $i + 1 ))
done 
